...
class RecommendationModel:
    ...
    def train(self, data_path="data/sample_students.csv"):
        ...
        if not SKLEARN_AVAILABLE:
            logger.info("scikit-learn is not available. Cannot train model. Centroid fallback will be used.")
            self.use_fallback = True
            return True
...
class PersonalityRecommendationModel:
    def __init__(self, model_path="models_saved/personality_rf_model.joblib"):
        self.model_path = model_path
        self.model = None
        self.feature_names = [
            "analytical_thinking", "creativity", "curiosity", "attention_to_detail",
            "communication", "leadership", "building_mindset", "research_mindset",
            "user_empathy", "problem_solving", "technical_depth"
        ]
        self.use_fallback = not SKLEARN_AVAILABLE
        self._load_model()
    ...
    def _load_model(self):
        if self.use_fallback:
            logger.info("Using profile-centroid personality model (fallback mode).")
            return

        if os.path.exists(self.model_path):
            try:
                self.model = joblib.load(self.model_path)
                logger.info("Personality recommendation model loaded successfully.")
            except Exception as e:
                logger.error(f"Failed to load personality model: {e}. Switching to centroid fallback.")
                self.use_fallback = True
        else:
            logger.warning(f"Personality model not found at {self.model_path}. Switching to centroid fallback.")
            self.use_fallback = True

    def train(self, data_path="data/preprocessed_psychometrics.csv"):
        """Trains the Random Forest model using the preprocessed psychometrics data."""
        if not SKLEARN_AVAILABLE:
            logger.info("scikit-learn is not available. Cannot train model. Centroid fallback will be used.")
            self.use_fallback = True
            return True
        ...
        try:
            ...
            os.makedirs(os.path.dirname(self.model_path), exist_ok=True)
            joblib.dump(self.model, self.model_path, compress=3)
            logger.info(f"Personality model trained and saved to {self.model_path}")
            self.use_fallback = False
            return True
        except Exception as e:
            logger.error(f"Error training personality model: {e}. Falling back to centroid-based model.")
            self.use_fallback = True
            return True

    def predict(self, scores: dict):
        """Predicts the career domain based on personality/aptitude scores."""
        if self.use_fallback:
            return self._predict_fallback(scores)

        if not self.model:
            raise ValueError("Personality model is not loaded or trained.")
        ...
        try:
            ...
            # Return recommendations with confidence scores out of 100
            recommendations = [
                {"domain": domain, "score": float(round(prob * 100, 2))}
            ...
            logger.error(f"Prediction using personality RF failed: {e}. Trying fallback predictor.")
            return self._predict_fallback(scores)

    def _predict_fallback(self, scores: dict, data_path="data/preprocessed_psychometrics.csv"):
        """Centroid-based similarity fallback algorithm for personality/aptitude model."""
        try:
            if not os.path.exists(data_path):
                logger.warning(f"Data file not found at {data_path}. Using default personality recommendations.")
                return {
                    "recommendations": [
                        {"domain": "AI/ML", "score": 25.0},
                        {"domain": "Web Development", "score": 20.0},
            ...
            logger.error(f"Personality fallback prediction failed: {err}")
            return {
                "recommendations": [
                    {"domain": "AI/ML", "score": 20.0},
                    {"domain": "Web Development", "score": 20.0},
...
personality_recommender = PersonalityRecommendationModel()
